    ...
    async.eachSeries(config.selector, function (item, callback) {
        ...
        if (index === config.selector.length - 1) {
            if (config.type) {
                if (that[config.type]) {
					if(config.type=='json'){
						//that[config.type](urlLevels[0]);
					}else{
						that[config.type](urlLevels[index - 1]);
					}
                } else {
                    that.log('参数type值无效，参数为text|image|json', 'redBG');
                }
            } else {
                that.log('您没有配置参数type，参数为text|image|json', 'redBG');
            }
        } 
                /// 第一层级
        else if (index === 0) {
...
/// 处理json
/// urls:{Array}
Crawler.prototype.json = function (urls) {
    var that = this;
    that.log('抓取文本中...');
    var i = 0;
	var count = urls.length;
			...
			async.whilst(function () {
                ...
                that.request(uri, function (status, $) {
                    if (status) {
                        ...
                        var last = config.selector;
						
						var obj={};
						var tmpLast="";
						
						for(j=1;j<config.selector.length;j++){
							tmpLast=last[j];
							if(tmpLast['attr']=='null'){
								obj[tmpLast['key_name']]=eval(tmpLast.$).text();
							}else{
								obj[tmpLast['key_name']]=new Array()
								eval(tmpLast.$).each(function(){
									tmp=$(this).attr(tmpLast['attr']);
									obj[tmpLast['key_name']].push(tmp);
								});
							}
						}
...
/// u:{String} 页面地址
/// callback:{Function} 获取页面完成后的回调callback(boolen,$)
/// m:{int} 初始化重试次数起点 默认最大为5
Crawler.prototype.spider=function(u,cb,m=1){
	ulink=url.parse(u);
	ulink['timeout']=30000;
	var req=http.get(ulink,function(res){
		var d = [];
		...
		res.on('end',function(){
			var bufferHtmlData =Buffer.concat(d,htmlDataLength);
			cb(bufferHtmlData);
		});
	});
	//超时,重试5次
