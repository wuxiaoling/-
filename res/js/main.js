...
exec.edit = function () {
    ...
    $.ajax({
        url: '/config/edit',
        data: { name:self.parent().data("name").toString() },
        cache: false,
        success: function (json) {
            if (json.status) {
...
/// 执行爬虫
exec.start = function () {
    var configname = $(this).parent().data('name').toString();
    var wrap = $('#Wrap');
    var loglist = $('#LogList');
    var fb = $('#FooterBar');
...
function validation(model) {
    ...
    for (var i in model) {
        if (!/^(charset)|(remove)|(imageFn)$/.test(i)) {
            if ('levels' === i) {
                for (var j = 0, len = model[i].length; j < len; j++) {
                    if ($.trim(model[i][j].key_name) === '') {
                        return j + 1 + '级页面key不能为空';
        ...
        if (i == 'configName'  && !isNaN(model[i].substring(0,1))) {
           return mapping[i] + '开头不能是数字和空';
        }
    }
